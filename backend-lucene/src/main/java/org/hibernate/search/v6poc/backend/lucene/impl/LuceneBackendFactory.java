...
public class LuceneBackendFactory implements BackendFactory {
	...
	private static final ConfigurationProperty<Optional<String>> TYPE = ConfigurationProperty.forKey( "lucene.backend_type" )
			.asString()
			.build();
	...
	public Backend<?> create(String name, BuildContext context, ConfigurationPropertySource propertySource) {
		...
		Optional<String> typeProperty = TYPE.get( propertySource );

		if ( !typeProperty.isPresent() ) {
			throw log.undefinedLuceneBackendType( name );
		}

		String type = typeProperty.get();

		if ( "local_directory".equals( type ) ) {
		...
		throw log.unrecognizedLuceneBackendType( name, type );
	}
}
